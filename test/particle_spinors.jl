...
@testset "particle spinors" for LorentzVectorType in [SFourMomentum, MFourMomentum]
    ...
    @testset "diracs equation" begin
        ...
        for spin in SPINS
            @test isapprox((slashed(P) - mass*one(DiracMatrix))*U[spin],zero(BiSpinor),atol=ATOL)
            @test isapprox((slashed(P) + mass*one(DiracMatrix))*V[spin],zero(BiSpinor),atol=ATOL)
            @test isapprox(Ubar[spin]*(slashed(P) - mass*one(DiracMatrix)),zero(AdjointBiSpinor),atol=ATOL)
            @test isapprox(Vbar[spin]*(slashed(P) + mass*one(DiracMatrix)),zero(AdjointBiSpinor),atol=ATOL)
        end
